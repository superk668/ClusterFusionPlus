/**
 * ClusterFusion Pythia-6.9B Decoder Layer Kernel
 * 
 ...
 * 
 * Key advantage over 2.8B: HEAD_DIM=128 (power of 2) enables:
 *   - 100% warp efficiency (all 128 threads work)
 *   - Cleaner thread mapping without bounds checks
...
__global__ void __cluster_dims__(CLUSTER_SIZE, 1, 1) Pythia6b9DecoderLayerKernel(
    // Output tensors
    ...
    const float softmax_scale = 0.12749325478956765f;  // 1/sqrt(128) * log2(e)
    ...
    // Weight layout: [Q0,K0,V0,Q1,K1,V1,...] where each has HEAD_DIM=128 dims
    // With HEAD_DIM=128, all threads are utilized (100% warp efficiency)
    ...
    for (int id = 1; id < DIM_PER_BLOCK / TMA_LOAD_ONCE; id++) {
        ...
        // No bounds check needed: weight_idx < HEAD_DIM=128 always true for 128 threads
    ...
    // Only first ROTARY_DIM=32 dimensions get rotary embedding
    if (tid < ROTARY_DIM) {
    ...
    // HEAD_DIM=128 means perfect alignment with 16 threads * 8 elements
    local_sum = 0.0f;
    for (int i = 0; i < NUM_PER_THREAD; i++)
        reg_reduce[i] = 0.0f;

    // Load Q into registers (no bounds check needed for HEAD_DIM=128)
