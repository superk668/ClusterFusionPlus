/**
 * CUDA Graph-compatible dispatch for Pythia-6.9B decoder layer.
 * 
...
extern __global__ void __cluster_dims__(CLUSTER_SIZE, 1, 1) Pythia6b9DecoderLayerKernel(
    half* output,
    half* k_output,
    half* v_output,
    half* input,
...
static std::unordered_map<int64_t, Pythia6b9GraphContext> g_contexts_6b9;
...
void pythia_6b9_create_graph_context_sm120(
    ...
    auto& ctx = g_contexts_6b9[context_id];
    ...
    // QKV weight tensor map: [hidden_dim, 3 * num_heads * head_dim] = [4096, 12288]
    ...
    // K cache tensor map - use max_seq_len: [hidden_dim, max_seq_len] = [4096, max_seq_len]
    ...
    // Output projection weight tensor map: [hidden_dim, hidden_dim] = [4096, 4096]
    ...
    // MLP up weight tensor map: [hidden_dim, ffn_dim] = [4096, 16384]
    ...
    // MLP down weight tensor map: [ffn_dim, hidden_dim] = [16384, 4096]
    ...
    cudaFuncSetAttribute(Pythia6b9DecoderLayerKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, ctx.max_shmem_size);
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> pythia_6b9_graph_decode_step_sm120(
    ...
    if (!ctx.initialized) {
        throw std::runtime_error("Context not initialized. Call pythia_6b9_create_graph_context first.");
    }
...
void pythia_6b9_destroy_graph_context_sm120(int64_t context_id) {
    g_contexts_6b9.erase(context_id);
}
